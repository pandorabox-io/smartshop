...
smartshop.minegeldtonumber = function(stack)
	-- return number of minegeld in stack, returns nil if
...
smartshop.send_mail = function(owner, pos, item)
	...
	mail.send("DO NOT REPLY", owner, "Out of " .. smartshop.get_human_name(item)
		.. " at " .. spos, "Your smartshop at " .. spos .. " is out of "
		.. smartshop.get_human_name(item) .. ". Please restock")
end

smartshop.receive_fields = function(player, pressed)
	if pressed.customer then
		return smartshop.showform(
			smartshop.user[player:get_player_name()], player, true)
	...
	elseif not pressed.quit then
		...
		if pressed["buy" .. n] then
			...
			local pay = inv:get_stack("pay" .. n, 1):get_name() .. " "
				.. inv:get_stack("pay" .. n, 1):get_count()
			if name ~= "" then
				if type == 1 and inv:room_for_item("main", pay) == false then
					minetest.chat_send_player(pname,
						"Error: The owner's stock is full, can't receive, "
						.. "exchange aborted.")
					return
				end
				...
				if meta:get_int("ghost") ~= 1 then
					...
					for i = 1, 4 do
						if inv:room_for_item("main", "pay" .. i)
							and inv:room_for_item("main", "give" .. i)
						then
				...
				if type == 1 and inv:contains_item("main", stack) == false then
					minetest.chat_send_player(pname,
						"Error: " .. smartshop.get_human_name(name) .. " is sold out.")
				...
				if not pinv:contains_item("main", pay) then
					minetest.chat_send_player(pname,
						"Error: You don't have enough in your inventory to buy "
						.. "this, exchange aborted.")
					return
				end
...
smartshop.update_info = function(pos)
	...
	if meta:get_int("type") == 0 then
		meta:set_string("infotext", "(Smartshop by " .. owner
			.. ") Stock is unlimited")
		return false
	end
	...
	for i = 1, 4, 1 do
		...
		if stuff["name" .. i] == "" or stuff["buy" .. i] == 0 then
			...
			smartshop.itemsatpos(spos, stuff["name" .. i],
				stuff["buy" .. i] * stuff["count" .. i])
...
smartshop.update = function(pos, stat)
	...
	for _, ob in ipairs(minetest.env:get_objects_inside_radius(pos, 2)) do
		if ob and ob:get_luaentity() and ob:get_luaentity().smartshop
			and ob:get_luaentity().pos == spos
		then
			ob:remove()
		end
	end
...
smartshop.showform = function(pos, player, re)
	...
	if owner then
		...
		if creative == 1 then
			gui = gui
				.. "label[0.5, -0.4;Your stock is unlimited because you have "
				.. "creative or give]"
...
--				minetest.chat_send_all(sellitem)
	...
	allow_metadata_inventory_move = function(pos, from_list, from_index, to_list, to_index, count, player)
		...
		then
			...
			if (string.find(from_list, "pay") or string.find(from_list, "give"))
				and to_list == "main"
			then
				inv:set_stack(from_list, from_index, ItemStack(""))
				return 0

			elseif (string.find(to_list, "pay") or string.find(to_list, "give"))
				and from_list == "main"
			then
	...
	can_dig = function(pos, player)
		...
		if ((meta:get_string("owner") == player:get_player_name()
			or minetest.check_player_privs(player:get_player_name(), { protection_bypass = true })) and inv:is_empty("main") and inv:is_empty("pay1") and inv:is_empty("pay2") and inv:is_empty("pay3") and inv:is_empty("pay4") and inv:is_empty("give1") and inv:is_empty("give2") and inv:is_empty("give3") and inv:is_empty("give4"))
...
smartshop.get_item_price = function(name)
	local sum = smartshop.get_item_count(name)
...
if false then
-- This lbm is used to add pre-update smartshops to the price database. Activate with care! Warning: very slow.
